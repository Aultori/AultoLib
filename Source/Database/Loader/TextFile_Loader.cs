...
namespace AultoLib.Database
{
    public static class TextFile_Loader
    {
        public static bool TryGetContents(SocietyDef societyDef, string path, out string rawText)
        {
            ...
            if (!DataIsLoaded(societyDef.KeyUpper))
            {
                TextFile_Loader.Load(societyDef);
            }
            if (GrammarDatabase.loadedTextFiles[societyDef.KeyUpper]?.TryGetValue(path, out rawText) == true)
            {
                // Log.Message($"{Globals.DEBUG_LOG_HEADER} loaded {rawText}");
                return true;
            }

            AultoLog.DebugMessage($"file {path} not found in the {societyDef.defName}");
            // look for it in the fallback society
            if (!DataIsLoaded(SocietyDefOf.fallback.KeyUpper))
                TextFile_Loader.Load(SocietyDefOf.fallback);
            if (GrammarDatabase.loadedTextFiles[SocietyDefOf.fallback.KeyUpper]?.TryGetValue(path, out rawText) == true)
                return true;
        ...
        {
            if (DataIsLoaded(societyDef.KeyUpper)) return;

            // Log.Message($"{Globals.DEBUG_LOG_HEADER} Attempting to load data");
            // data isn't loaded, so
            // path --> fileData
            GrammarDatabase.loadedTextFiles[societyDef.KeyUpper] = new Dictionary<string, string>();
            //
            DeepProfiler.Start($"{Globals.LOG_HEADER} Loading society data: {societyDef.defName} ({societyDef.KeyUpper})");
            try
            {
        ...
        private static bool TryRegisterFileIfNew(SomeModDirectory modDir, string filePath, SocietyDef society)
        {
            ...
            if (!DataIsLoaded(society.KeyUpper))
            {
        ...
        private static void LoadFile(VirtualFile file, VirtualDirectory stringsTopDir, SocietyDef societyDef)
        {
            ...
            LoadToDatabase(societyDef.KeyUpper, shortPath, allText);
        }
