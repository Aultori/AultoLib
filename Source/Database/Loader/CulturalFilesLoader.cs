...
namespace AultoLib.Database
{
    ...
    /// </summary>
    public static class CulturalFilesLoader
    {
        ...
        public static void Load(CultureDef culture)
        {
            ...
            try
            {
                CulturalFilesLoader.tmpAlreadyLoadedFiles.Clear();
                
                foreach (SomeModDirectory modDir in GetDirectories(culture.MainPath))
                {
                    SomeModDirectory localDir = modDir;
                    if (!CulturalFilesLoader.tmpAlreadyLoadedFiles.ContainsKey(localDir.mod))
                    {
                        // create new element if it doesn't exist already
                        CulturalFilesLoader.tmpAlreadyLoadedFiles[localDir.mod] = new HashSet<string>();
                    }
