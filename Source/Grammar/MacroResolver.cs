...
namespace AultoLib.Grammar
{
    public static class MacroResolver
    {
        ...
        public static bool TryResolve(string rootKeyword, string debugLabel, out string expansion)
        {
            ...
            logSb.Append($"{Globals.RESOLVER_HEADER} Trying to resolve {debugLabel}");
            ...
            if (doLog) AultoLog.DebugMessage_Advanced("expansion successful");
            return true;
        }
        ...
        {
            loopCount++;
            if (doLog) AultoLog.DebugMessage_Advanced($"DOING RECURSION: level {r}, loop {loopCount}, segments: \"{segments}\"");
            ...
            foreach (RuleSegments.Segment seg in segments.List)
            {
                if (seg.isMacro)
                {
                    if (doLog) AultoLog.DebugMessage_Advanced($"expanding segment \"{seg}\"");
                    ...
                    if (TryExpandMacro(seg.macro, out CompoundRule compoundRule))
                    {
                        ...
                        if (rule == null)
                        {
                            if (doLog) AultoLog.DebugMessage_Advanced($"selected rule, but it was null");
                            yield break;
                        }

                        if (doLog) AultoLog.DebugMessage_Advanced($"selected rule {rule.keyword}");
                        ...
                        // expand rule
                        RuleSegments segs = rule.GetSegments();
                        if (doLog) AultoLog.DebugMessage_Advanced($"got segments: {segs}");
                ...
                {
                    if (doLog) AultoLog.DebugMessage_Advanced($"expanding text \"{seg}\"");
                    yield return seg.text;
                }
            }
            yield break;
