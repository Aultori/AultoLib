...
namespace AultoLib.Grammar
{
    public class Ruleset
    {
        ...
        public Dictionary<string, CompoundRule> RulesRaw
        {
            get
            {
        ...
        public Dictionary<string, CompoundRule> RulesPlusDefs
        {
            get
            {
                ...
                { 
                    ...
                    this.rulesPlusDefs.AddDict(this.RulesRaw);
        ...
        {
            keyword = null;
            if (rawRules == null) return false;
            if (rawRules.Count == 0) return false;
        ...
        public void ResolveReferences()
        {
            ...
            {
                Log.Error($"{Globals.LOG_HEADER} Cannot resolve the references of a Ruleset not created by a def");
                return;
            }
            ...
            {
                this.resolvedRules.AddDict(rulesetDef.ruleset.RulesRaw); // useActiveSociety will never be set
                this.resolvedRules.AddDict(rulesetDef.ruleset.RulesResolved);
            }
            this.rulesPlusDefs = new RuleCollection();
            this.rulesPlusDefs.AddDict(this.RulesRaw); // initializes cached rules if there isn't anything there
            this.rulesPlusDefs.Add(this.resolvedRules);

            Log.Message($"{Globals.DEBUG_LOG_HEADER} resolved some refferecnes");
        }
        ...
        private bool CanEdit()
        {
            ...
            if (this.initialized)
            {
                Log.Error($"{Globals.LOG_HEADER} Cannot add to a Ruleset created by a Def");
                return false;
            }
            return true;
        }
        ...
        public class RuleCollection
        {
            ...
            {
                foreach (CompoundRule compoundRule in ruleCollection.data.Values)
                   this.Add(compoundRule);
            }

            public void AddDict(Dictionary<string, CompoundRule> dict)
            {
                foreach (CompoundRule compoundRule in dict.Values)
                    this.Add(compoundRule);
            }
