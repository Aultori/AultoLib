...
namespace AultoLib.Grammar
{
    public class Ruleset
    {
        ...
        {
            if (AultoLog.DoLog()) AultoLog.Message($"resolving references");
            if (this.rulesStrings == null && this.rulesFiles == null)
            {
                AultoLog.Error($"Cannot resolve the references of a Ruleset with no {nameof(rulesStrings)} or {nameof(rulesFiles)} to resolve");
                return;
            }
        ...
        private bool CanEdit()
        {
            ...
            if (this.isReadonly)
            {
                AultoLog.Error("Cannot add to a Ruleset that's set to readonly");
                return false;
            }
            return true;
        }
